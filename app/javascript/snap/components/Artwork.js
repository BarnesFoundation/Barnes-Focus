...
class Artwork extends Component {
  ...
  constructResultAndInRoomSlider = search_result => {
    if (search_result["success"]) {
      ...
      if (search_result["data"]["records"].length > 0) {
        ...
        const topCropParams =
          "?q=0&auto=compress&crop=top&fit=crop&h=" + h + "&w=" + w;
        ...
        result["unIdentified"] = art_obj.people
          .toLowerCase()
  ...
  onSelectLanguage = async lang => {
    console.log(
      "Selected lang changed in Artwork >> : " + JSON.stringify(lang)
    );
  ...
  componentDidUpdate() {
    ...
    if (this.infoCardRef) {
      console.log(
        "this.infoCardRef.getBoundingClientRect().height : ",
        this.infoCardRef.getBoundingClientRect().height
      );
    }
  }
  ...
  handleScroll = () => {
    ...
    const resultsContainerBottom = Math.ceil(
      this.artworkRef.getBoundingClientRect().bottom - constants.VIEWPORT_HEIGHT
    );
    this.scrollInProgress = false;
  };
  ...
  setArtworkRef = elem => {
    if (elem) {
      ...
      console.log(
        "setArtworkRef >> artworkVScrollOffset == ",
        artworkVScrollOffset
      );
      this.setState({ infoCardDuration: artworkVScrollOffset });
      console.log(
        "setArtworkRef >> infoCardDuration = ",
        this.state.infoCardDuration
      );
    }
  };
  ...
  renderEmailScreen = () => {
    ...
    if (emailCaptureAck) {
      ...
      return (
        <div
          id="email-panel"
          ref={this.emailCardRef}
          className="panel panel-email"
        >
          <EmailForm
            withStory={showStory}
            isEmailScreen={false}
  ...
  renderStory = () => {
    ...
    return stories.map((story, index) => (
      <Scene
        ...
        duration={this.state.storyDurationsCurrent[index] * 4}
        offset={
          index > 0
            ? this.state.storyOffsets[index] - 375
            : this.state.infoCardDuration + this.contentOffset - 100
        }
      >
        {(progress, event) => (
      ...
      <Scene
        ...
        duration={
          index > 0
            ...
            : this.state.infoCardDuration + this.contentOffset
        }
        offset="0"
  ...
  renderPinsLeave = () => {
    ...
    return stories.map((story, index) => (
      <Scene
        ...
        offset={
          index > 0
            ? this.state.storyOffsets[index] - 100
            : this.state.infoCardDuration + this.contentOffset - 100
        }
  ...
  renderResult = () => {
    ...
    const artworkOffset =
      this.state.infoCardDuration + this.contentOffset + extraOffset;
    console.log(
      "Artwork render >> Artwork Scene duration : ",
      this.state.infoCardDuration,
      this.contentOffset,
      extraOffset,
      artworkOffset
      ...
      <SectionWipesStyled hasChildCards={hasChildCards}>
        <Controller refreshInterval={50}>
          ...
          <Scene
            ...
            pinSettings={{ pushFollowers: true }}
          >
            {(progress, event) => this.renderArtwork(progress)}
          </Scene>
