...
class ImageUploadJob < ApplicationJob
  ...
  def perform(*args)
    ...
    upload = Upload.find_by id: args[0].to_i
    if upload
      image_data = Base64.decode64(upload.blob['data:image/png;base64,'.length .. -1])
      ...
      if File.exists?(file.path)
        ...
        snap_search_result.searched_image_data = upload.blob
      end
